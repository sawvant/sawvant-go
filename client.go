...
// APIClient manages communication with the Sawvant Cutting Optimization API API v1.0.0
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	c.OptimizationAPI = (*OptimizationAPIService)(&c.common)
...
func (c *APIClient) callAPI(request *http.Request) (*http.Response, error) {
	...
	if resp.StatusCode == 429 {
		...
		func() {
			...
			if jsonErr := json.Unmarshal(bodyBytes, &body); jsonErr != nil || body.Reset == nil {
				return
			}
			switch v := body.Reset.(type) {
			case float64:
				delay := time.Until(time.Unix(int64(v), 0))
				if delay > 0 {
					waitDuration = delay
				}
			case string:
